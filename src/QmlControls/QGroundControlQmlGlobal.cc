...
void QGroundControlQmlGlobal::setToolbox(QGCToolbox* toolbox)
{
    ...
    _monarkManager          = toolbox->monarkManager();
...
QString QGroundControlQmlGlobal::qgcVersion(void) const
{
    ...
    versionStr += QStringLiteral(" %1").arg(tr("EchoMAV"));
#ifdef __androidArm32__
    versionStr += QStringLiteral(" %1").arg(tr("32 bit EchoMAV"));
#elif __androidArm64__
    versionStr += QStringLiteral(" %1").arg(tr("64 bit EchoMAV"));
#endif
    return versionStr;
}
